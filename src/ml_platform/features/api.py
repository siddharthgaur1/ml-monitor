"""REST API for ml_platform.features, built on FastAPI.

Run with: uvicorn ml_platform.features.api:app
...
from ml_platform.features.quality.validator import DataQualityError
from ml_platform.features.registry.validator import SchemaValidationError

app = FastAPI(title="ml_platform.features", version="0.1.0")
...
def get_store() -> FeatureStore:
    global _fs
    if _fs is None:
        _fs = FeatureStore(config=os.environ.get("FEATURESTORE_CONFIG", "ml_platform.yaml"))
    return _fs
