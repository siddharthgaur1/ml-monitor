"""Command-line interface for ml_platform.features."""
...
from ml_platform.features.materialization.materializer import materialize as materialize_fn
from ml_platform.features.quality.validator import DataQualityError, check_quality
from ml_platform.features.registry.validator import SchemaValidationError, validate_schema
...
@click.group()
@click.option("--config", default="ml_platform.yaml", show_default=True, help="Path to ml_platform.yaml")
@click.pass_context
def cli(ctx, config):
...
@click.pass_context
def init(ctx):
    """Scaffold a ml_platform.yaml and data/ directory in the current dir."""
    config_path = Path(ctx.obj["config"])
    if config_path.exists():
        console.print(f"[yellow]{config_path} already exists, skipping[/yellow]")
    else:
        example = Path(__file__).parent.parent / "ml_platform.yaml.example"
        if example.exists():
