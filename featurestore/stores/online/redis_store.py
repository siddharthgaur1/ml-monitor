...
Key pattern: featurestore:{feature_group}:{entity_id} -> msgpack-encoded
dict of {feature_name: value}. Batch reads use a Redis pipeline. Per-group
TTL is applied via Redis EXPIRE.

Optional extra: pip install lite-featurestore[redis]. Importing this module
...
def _key(group_name: str, entity_id) -> str:
    return f"featurestore:{group_name}:{entity_id}"
...
class RedisOnlineStore(OnlineStore):
    def __init__(self, host: str = "localhost", port: int = 6379, ttl_hours: float = 24, **kwargs):
        if redis is None:
            raise ImportError(
                "redis is not installed. Install with: pip install lite-featurestore[redis]"
            )
