...
from featurestore.quality.profiler import profile
from featurestore.quality.validator import check_quality
from featurestore.registry.registry import FeatureRegistry
from featurestore.registry.validator import validate_schema
...
def _build_offline_store(cfg):
    if cfg.type == "parquet":
        from featurestore.stores.offline.parquet_store import ParquetOfflineStore
        return ParquetOfflineStore(path=cfg.get("path", "./data/offline"))
    if cfg.type == "duckdb":
        from featurestore.stores.offline.duckdb_store import DuckDBOfflineStore
...
def _build_online_store(cfg):
    if cfg.type == "sqlite":
        from featurestore.stores.online.sqlite_store import SQLiteOnlineStore
        return SQLiteOnlineStore(path=cfg.get("path", "./data/online.db"), ttl_hours=cfg.get("ttl_hours", 24))
    if cfg.type == "redis":
        from featurestore.stores.online.redis_store import RedisOnlineStore
...
class FeatureStore:
    def __init__(self, config: str | FeatureStoreConfig = "featurestore.yaml"):
